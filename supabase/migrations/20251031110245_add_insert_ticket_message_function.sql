/*
  # Add function to insert ticket messages (bypasses PostgREST cache)

  1. New Functions
    - `insert_ticket_message` - Inserts a message and returns it
      - Bypasses PostgREST schema cache issues
      - Updates parent ticket timestamp
      - Parameters: ticket_id, message, is_admin, user_id (optional)
  
  2. Security
    - Function runs with SECURITY DEFINER (uses function owner's permissions)
