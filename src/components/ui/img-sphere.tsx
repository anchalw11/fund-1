 ...
 * - Adapted to match the website's cyberpunk color theme
 *
 * Usage:
 * ```tsx
 * <SphereImageGrid
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-electric-blue/30 hover:border-electric-blue/60 transition-all duration-300">
          <img
            src={image.src}
            alt={image.alt}
          ...
          <div className="absolute inset-0 rounded-full bg-gradient-to-br from-electric-blue/20 via-cyber-purple/20 to-neon-green/20 opacity-0 hover:opacity-100 transition-opacity duration-300" />
        </div>
      </div>
    );
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-gray-900/95 rounded-xl max-w-md w-full overflow-hidden border border-electric-blue/30 shadow-2xl shadow-electric-blue/20"
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 w-8 h-8 bg-black/80 border border-electric-blue/50 rounded-full text-electric-blue flex items-center justify-center hover:bg-electric-blue/20 transition-all cursor-pointer"
            >
              <X size={16} />
            </button>
            {/* Cyberpunk corner accents */}
            <div className="absolute top-0 left-0 w-4 h-4 border-l-2 border-t-2 border-electric-blue opacity-60" />
            <div className="absolute top-0 right-0 w-4 h-4 border-r-2 border-t-2 border-electric-blue opacity-60" />
            <div className="absolute bottom-0 left-0 w-4 h-4 border-l-2 border-b-2 border-electric-blue opacity-60" />
            <div className="absolute bottom-0 right-0 w-4 h-4 border-r-2 border-b-2 border-electric-blue opacity-60" />
          </div>
  ...
  if (!isMounted) {
    return (
      <div
        className="bg-gray-900/50 rounded-lg animate-pulse flex items-center justify-center border border-electric-blue/20"
  ...
  if (!images.length) {
    return (
      <div
        className="bg-gray-900/50 rounded-lg border-2 border-dashed border-electric-blue/30 flex items-center justify-center"
        style={{ width: containerSize, height: containerSize }}
      >
