...
export default function AccountCard({ account, onClick }: AccountCardProps) {
  ...
  const getStatusBadge = () => {
    ...
    } else if (account.status === 'breached') {
      return <span className="px-3 py-1 rounded-full text-xs font-semibold bg-gray-500/20 text-gray-400 border border-gray-500/30">Breached</span>;
    } else {
  ...
  return (
    <button
      onClick={isBreached ? undefined : onClick}
      disabled={isBreached}
      className={`w-full bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm rounded-xl p-5 border transition-all duration-300 text-left group ${
        isBreached
          ? 'border-gray-600 bg-gray-800/50 opacity-60 cursor-not-allowed'
          : 'border-white/10 hover:border-electric-blue/50 hover:shadow-lg hover:shadow-electric-blue/10'
      }`}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${
            isBreached ? 'bg-gray-700/50 border-gray-600' : 'bg-electric-blue/20 border-electric-blue/30'
          }`}>
            {isBreached ? (
