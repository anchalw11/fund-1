...
export const pricingService = {
  async getChallengeTypes() {
    try {
      const { data, error } = await db.queryAll('challenge_types');

      if (error) throw error;
      
      const activeChallenges = data.filter((c: any) => c.is_active);
      activeChallenges.sort((a: any, b: any) => a.sort_order - b.sort_order);

      return activeChallenges as ChallengeTypeDB[];
    } catch (error) {
