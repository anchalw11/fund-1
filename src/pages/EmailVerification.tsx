        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <p className="text-center text-gray-600 mb-8">
            {success
              ...
              : `Account created successfully. Use the verification code below to continue.`}
          </p>

          {!success && !verificationSent && storedCode && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-2 mb-2">
                <AlertCircle className="w-5 h-5 text-yellow-600" />
                <span className="font-medium text-yellow-800">Testing Mode</span>
              </div>
              <p className="text-sm text-yellow-700 mb-3">
                Email verification is not configured. Use this code to complete verification:
              </p>
              <div className="bg-white border-2 border-dashed border-yellow-300 rounded-lg p-4 text-center">
                <div className="text-3xl font-mono font-bold text-yellow-800 tracking-wider">
                  {storedCode}
                </div>
                <p className="text-xs text-yellow-600 mt-2">Click to copy</p>
              </div>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(storedCode);
                  alert('Code copied to clipboard!');
                }}
                className="w-full mt-3 bg-yellow-600 text-white py-2 rounded-lg font-medium hover:bg-yellow-700 transition-colors"
              >
                Copy Code
              </button>
            </div>
