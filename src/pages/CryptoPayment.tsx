...
export default function CryptoPayment() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'crypto' | 'razorpay'>('crypto');
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<'mt5' | 'tradingview'>('mt5');
  ...
  // Calculate platform fee for TradingView (now free)
  const platformFee = 0;

  // Add $5 if competition add-on is selected, plus platform fee
  const finalPrice = couponDiscountedPrice + (challengeType === 'COMPETITION' && hasAddOn ? 5 : 0) + platformFee;
  ...
  // Check if Razorpay is available for this challenge type and account size
  const isRazorpayAvailable = () => {
    if (challengeType === 'COMPETITION') return true;
    if (challengeType === 'MINI_PAID') return true; // Mini challenge $0.99 payment is always available via Razorpay

    if (challengeType === 'CLASSIC_2STEP') {
      return [5000, 10000, 25000, 50000].includes(accountSize);
    }

    if (challengeType === 'RAPID_FIRE') {
      return [5000, 10000, 25000, 50000, 100000].includes(accountSize);
    }

    if (challengeType === 'AGGRESSIVE_2STEP') {
      return [5000, 10000, 25000, 50000, 100000, 200000].includes(accountSize);
    }

    if (challengeType === 'SWING_PRO') {
      return [5000, 10000, 25000, 50000, 100000, 200000].includes(accountSize);
    }
  ...
  // Razorpay payment handler - Now creates order first, then lets webhook handle verification
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      // First, create an order through our backend with payment metadata
      ...
      // Now configure Razorpay with the order
      const options = {
        ...
        handler: function (response: any) {
          ...
          // Now show processing message and let webhook handle verification
          setVerificationStatus('checking');
          ...
          const checkPaymentStatus = async () => {
            ...
            try {
              // Check if user challenge has been created by webhook
              if (!supabase) {
                setVerificationStatus('failed');
                alert('Database connection unavailable. Please contact support.');
                setVerifying(false);
                return;
              }
              ...
              const { data: challenges, error } = await supabase
                ...
                .select('id, status, created_at, challenge_type')
                .eq('user_id', user.id)
                .eq('challenge_type', typeNameMap[challengeType] || challengeType.toLowerCase())
                .eq('account_size', challengeType === 'COMPETITION' ? 100000 : accountSize)
                .gte('created_at', new Date(Date.now() - 2 * 60 * 1000).toISOString()) // Within last 2 minutes
                .order('created_at', { ascending: false })
                .limit(1);

              if (!error && challenges && challenges.length > 0) {
                const challenge = challenges[0];
                console.log('User challenge found via webhook:', challenge);

                // Success! Challenge was created by webhook
                ...
                if (window.twq) {
                  window.twq('event', 'tw-qq6v0-qq6v1', {
                    currency: 'USD',
                    conversion_id: response.razorpay_payment_id,
              ...
              // If not found and we haven't exceeded max attempts, check again
              if (attempts < maxAttempts) {
                setTimeout(checkPaymentStatus, checkInterval);
              } else {
                console.error('Payment webhook verification timed out');
                setVerificationStatus('failed');
                alert('Payment processing is taking longer than expected. Please check your dashboard in a few minutes or contact support.');
                setVerifying(false);
              }
            } catch (error) {
          ...
          setTimeout(checkPaymentStatus, 2000); // Initial delay to let webhook process
        },
        ...
        modal: {
          ...
          confirm_close: true,
          confirm_close_text: 'Are you sure you want to cancel this payment?',
          escape: true,
          animation: true
        },
        ...
        // Additional metadata that can be passed to webhook
        notes: {
          user_id: user.id,
          challenge_type: challengeType,
  ...
  const verifyPayment = async () => {
    ...
    try {
      ...
      if (isValid) {
        ...
        // Special handling for MINI_PAID challenges - create mini challenge instead of regular challenge
        if (challengeType === 'MINI_PAID') {
          // Create mini challenge directly
          const miniChallengeData = {
            user_id: user.id,
            email: user?.email,
            account_size: accountSize,
            duration_days: 14, // Extended duration for paid mini challenges
            profit_target: 200, // $200 profit target
            daily_loss_limit: accountSize * 0.05, // 5% daily loss limit
            status: 'active',
          ...
          console.log('Creating mini challenge:', miniChallengeData);

          const { data: miniChallenge, error: miniError } = await supabase
            .from('mini_challenges')
            .insert(miniChallengeData)
            .select()
            .single();

          if (miniError) {
            console.error('Failed to create mini challenge:', miniError);
            alert('Error creating mini challenge. Please contact support.');
            setVerificationStatus('failed');
            setVerifying(false);
            return;
          }

          console.log('Mini challenge created successfully:', miniChallenge);

          // Generate documents for mini challenge
          try {
            await supabase
              .from('downloads')
              .insert({
                user_id: user.id,
                challenge_id: miniChallenge.id,
                document_type: 'WELCOME_CERTIFICATE',
                title: 'Mini Challenge Welcome Certificate',
                description: 'Welcome to your paid mini challenge!',
                document_number: `MINI-WELCOME-${Date.now()}`,
            ...
            await supabase
              ...
              .insert({
                ...
                title: 'Mini Challenge Invoice',
                description: 'Invoice for mini challenge purchase',
                document_number: `MINI-INV-${Date.now()}`,
                issue_date: new Date().toISOString(),
                status: 'ready'
              });
          } catch (docError) {
          ...
          setTimeout(() => {
            navigate('/dashboard', {
              state: {
                ...
                miniChallengeId: miniChallenge.id
              }
            });
          }, 2000);
          return; // Exit early for mini challenges
        }
        ...
        {finalPrice > 0 && (
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Razorpay Link for Indians - Available for Competition and Classic challenges */}
              {isRazorpayAvailable() && (
                <button
                onClick={() => {
                  let razorpayLink = 'https://rzp.io/rzp/BHsokDPj'; // Default competition link

                  // MINI_PAID uses always the same $0.99 link
                  if (challengeType === 'MINI_PAID') {
                    razorpayLink = 'https://rzp.io/rzp/OAHoNOrn';
                  } else if (challengeType === 'CLASSIC_2STEP') {
                    if (accountSize === 5000) razorpayLink = 'https://rzp.io/rzp/4DgNKSZe';
                    else if (accountSize === 10000) razorpayLink = 'https://rzp.io/rzp/cLmzwo2O';
                    else if (accountSize === 25000) razorpayLink = 'https://rzp.io/rzp/xDwWiYIE';
                    else if (accountSize === 50000) razorpayLink = 'https://rzp.io/rzp/tU0FGwd';
                  } else if (challengeType === 'RAPID_FIRE') {
                    if (accountSize === 5000) razorpayLink = 'https://rzp.io/rzp/qTKnldsh';
                    else if (accountSize === 10000) razorpayLink = 'https://rzp.io/rzp/WG4Mn6II';
                    else if (accountSize === 25000) razorpayLink = 'https://rzp.io/rzp/bf3dENl';
                    else if (accountSize === 50000) razorpayLink = 'https://rzp.io/rzp/d3YjeKE0';
                    else if (accountSize === 100000) razorpayLink = 'https://rzp.io/rzp/AJv7r8n';
                  } else if (challengeType === 'AGGRESSIVE_2STEP') {
                    if (accountSize === 5000) razorpayLink = 'https://rzp.io/rzp/3shM2o5';
                    else if (accountSize === 10000) razorpayLink = 'https://rzp.io/rzp/W7UgvH2';
                    else if (accountSize === 25000) razorpayLink = 'https://rzp.io/rzp/l7NldIKh';
                    else if (accountSize === 50000) razorpayLink = 'https://rzp.io/rzp/zb8I1uoI';
                    else if (accountSize === 100000) razorpayLink = 'https://rzp.io/rzp/gzSESaE';
                    else if (accountSize === 200000) razorpayLink = 'https://rzp.io/rzp/0x05vdg';
                  } else if (challengeType === 'SWING_PRO') {
                    if (accountSize === 5000) razorpayLink = 'https://rzp.io/rzp/Fjrsa6uO';
                    else if (accountSize === 10000) razorpayLink = 'https://rzp.io/rzp/vMPkWSM';
                    else if (accountSize === 25000) razorpayLink = 'https://rzp.io/rzp/zRA9N0td';
                    else if (accountSize === 50000) razorpayLink = 'https://rzp.io/rzp/lpjnaOv';
                    else if (accountSize === 100000) razorpayLink = 'https://rzp.io/rzp/TOY1FIeM';
                    else if (accountSize === 200000) razorpayLink = 'https://rzp.io/rzp/QTcJn5d';
                  }

                  window.open(razorpayLink, '_blank');
                }}
                className={`group relative overflow-hidden p-8 rounded-xl border-2 transition-all duration-500 cursor-pointer ${
                  'border-orange-400 bg-gradient-to-br from-orange-500/10 to-red-500/10 shadow-2xl shadow-orange-500/20'
                }`}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-orange-600/5 to-red-600/5 opacity-100"></div>
                <div className="relative z-10">
                  <div className="flex items-center justify-center mb-4">
                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center ring-4 ring-orange-400/30 animate-pulse">
                      <CreditCard className="w-8 h-8 text-white" />
                    </div>
                  </div>
                  <h4 className="text-xl font-bold text-white mb-2 text-center">Razorpay (India)</h4>
                  <p className="text-sm text-gray-300 text-center mb-4">
                    Our Indian payment provider - best for Indians
                  </p>
                  <div className="flex flex-wrap justify-center gap-2 mb-4">
                    <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-full text-xs">UPI</span>
                    <span className="px-3 py-1 bg-blue-500/20 text-blue-400 rounded-full text-xs">Cards</span>
                    <span className="px-3 py-1 bg-purple-500/20 text-purple-400 rounded-full text-xs">Net Banking</span>
                  </div>
                  <div className="text-center">
                    <div className="inline-flex items-center px-4 py-2 bg-green-500/20 border border-green-400/50 rounded-lg">
                      <span className="text-green-400 text-sm font-bold">⭐ RECOMMENDED FOR INDIANS</span>
                    </div>
                  </div>
                  <div className="text-center mt-3">
                    <p className="text-xs text-gray-400">Click to pay with Razorpay</p>
                  </div>
                </div>
              </button>
              )}
              ...
              <div className="group relative overflow-hidden p-8 rounded-xl border-2 border-dashed border-gray-600 bg-gray-800/20 cursor-not-allowed opacity-60 pointer-events-none">
