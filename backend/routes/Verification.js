...
router.post('/send-code', async (req, res) => {
  try {
    ...
    if (!result.success && result.error === 'EMAIL_SEND_FAILED') {
      return res.status(400).json(result);
    }
    if (!result.success && result.error === 'DB_UPSERT_FAILED') {
  ...
  try {
    ...
    if (result.success && !result.alreadyVerified) {
      try {
        ...
        const user = {
          email: email,
          full_name: email.split('@')[0] // Use email prefix as name if no full name available
        };
