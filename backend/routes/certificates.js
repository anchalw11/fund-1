...
router.post('/welcome', async (req, res) => {
  ...
  try {
    ...
    console.log('📝 Generating welcome certificate for user:', user_id);
    ...
    const result = await certificateService.generateWelcomeCertificate(user_id, {
      full_name: user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || 'Trader',
      email: user.email
    });
...
router.post('/challenge-started', async (req, res) => {
  ...
  try {
    ...
    if (!user_id || !account_id) {
      console.error('❌ Missing user_id or account_id in request');
      return res.status(400).json({
        success: false,
    ...
    console.log('📝 Generating challenge started certificate for user:', user_id, 'account:', account_id);
    ...
    const result = await certificateService.generateChallengeStartedCertificate(user_id, {
      id: account_id,
      trader_name: user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || 'Trader',
      challenge_type: challenge_type || 'Trading Challenge',
      account_size: account_size || 10000,
      start_date: new Date().toISOString()
    });

    console.log('✅ Challenge started certificate generated successfully');
