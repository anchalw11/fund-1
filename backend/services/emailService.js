...
class EmailService {
  constructor() {
    // Initialize properties safely - don't call any methods that could fail
    this.transporter = null;
    this.isConfigured = false;
    this.templates = {};
    this.certsDir = null;

    // Defer initialization to avoid constructor failures
  ...
  async loadTemplates() {
    ...
    const templateNames = ['welcome', 'verification', 'signin'];
  ...
  initializeCertsDir() {
    try {
      ...
      console.log('✅ Certificates directory initialized');
    } catch (error) {
      console.error('❌ Failed to initialize certificates directory:', error.message);
      ...
      const html = `
        ...
        <body>
          <div class="email-container">
            ...
            <div class="footer">
              <p>Need help? Contact us at urgent.fund8r@gmail.com</p>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="warning">
              ...
              <ul>
                <li>Maximum Daily Loss: ${account.challenges?.max_daily_loss || 5}%</li>
                <li>Maximum Total Loss: ${account.challenges?.max_total_loss || 10}%</li>
                <li>Profit Target: $${account.profit_target?.toLocaleString() || 'N/A'}</li>
  ...
  async sendMT5CredentialsEmail(user, accountDetails) {
    const subject = '🚀 Your MT5 Account Credentials are Here!';
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
        <style>
          ...
          .credentials-box { background: #f8f9fa; border: 1px solid #e9ecef; border-left: 5px solid #667eea; padding: 20px; margin: 20px 0; border-radius: 5px; }
          .credential-item { padding: 10px 0; border-bottom: 1px solid #e9ecef; display: flex; justify-content: space-between; }
          .credential-item:last-child { border-bottom: none; }
          .credential-item strong { color: #495057; }
          .button { display: inline-block; padding: 15px 30px; background: #667eea; color: white; text-decoration: none; border-radius: 50px; margin-top: 20px; font-weight: bold; }
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>Your Challenge is Ready!</h1>
            <p>Here are your MT5 account credentials</p>
          </div>
          <div class="content">
            <h2>Hi ${user.first_name || 'Trader'},</h2>
            <p>Your <strong>${accountDetails.challenge_type}</strong> challenge with a <strong>$${accountDetails.account_size.toLocaleString()}</strong> account is now active. Use the credentials below to log in to your MT5 terminal.</p>
            
            <div class="credentials-box">
              <div class="credential-item"><strong>MT5 Login:</strong> <span>${accountDetails.account_number}</span></div>
              <div class="credential-item"><strong>Password:</strong> <span>${accountDetails.password}</span></div>
              <div class="credential-item"><strong>Server:</strong> <span>${accountDetails.server}</span></div>
            </div>

            <p><strong>Next Steps:</strong></p>
            <ol>
              <li>Download the MT5 platform if you haven't already.</li>
              <li>Log in using the credentials above.</li>
              <li>Start trading and meet your profit targets!</li>
            </ol>
          ...
          <div class="footer">
            ...
            <p>&copy; ${new Date().getFullYear()} ${process.env.COMPANY_NAME || 'Fund8r'}</p>
          </div>
        </div>
      </body>
      </html>
  ...
  async sendBreachEmail(user, breachData) {
    const subject = '❗ Account Breach Notification';
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; background-color: #f4f7f6; }
          .container { max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
          .header { background: linear-gradient(135deg, #d9534f 0%, #c9302c 100%); color: white; padding: 40px; text-align: center; }
          .content { padding: 30px; }
          .breach-box { background: #f2dede; border: 1px solid #ebccd1; border-left: 5px solid #a94442; padding: 20px; margin: 20px 0; border-radius: 5px; color: #a94442; }
          .button { display: inline-block; padding: 15px 30px; background: #667eea; color: white; text-decoration: none; border-radius: 50px; margin-top: 20px; font-weight: bold; }
          .footer { background: #e9ecef; padding: 20px; text-align: center; font-size: 12px; color: #6c757d; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2>Hi ${user.first_name || 'Trader'},</h2>
            <p>We are writing to inform you that your <strong>${breachData.challenge_type}</strong> challenge with a <strong>$${breachData.account_size.toLocaleString()}</strong> account has been breached and is no longer usable.</p>
            
            <div class="breach-box">
              <p><strong>Reason for Breach:</strong></p>
              <p>${breachData.reason}</p>
            </div>

            <p>This means the account can no longer be traded. You can view the details of the breach in your user dashboard.</p>
  ...
  async sendPhaseCompletionEmail(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Phase Completion Email Simulation (SMTP not configured)');
        console.log(`To: ${userData.email}`);
        console.log(`Phase: ${userData.phase}`);
        console.log(`Challenge ID: ${userData.challengeId}`);
        return;
      }
      ...
      const nextSteps = {
        1: 'Phase 2 credentials will be sent within 24 hours.',
        2: 'Your funded account credentials will be sent within 24 hours.'
      };

      const subject = `🎉 Congratulations! You Passed ${phaseNames[userData.phase]}!`;
      const html = `
        <!DOCTYPE html>
        <html>
        <head>
          <style>
            ...
            .content { padding: 30px; }
            .success-box { background: linear-gradient(135deg, #10B981 0%, #059669 100%); color: white; padding: 30px; border-radius: 10px; margin: 20px 0; text-align: center; }
            .phase-info { background: #f8f9fa; border: 1px solid #e9ecef; border-left: 5px solid #10B981; padding: 20px; margin: 20px 0; border-radius: 5px; }
            .button { display: inline-block; padding: 15px 30px; background: #10B981; color: white; text-decoration: none; border-radius: 50px; margin-top: 20px; font-weight: bold; }
            .footer { background: #e9ecef; padding: 20px; text-align: center; font-size: 12px; color: #6c757d; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="content">
              <h2>Hi ${userData.name || 'Trader'},</h2>
              <p>Fantastic news! You've successfully completed <strong>${phaseNames[userData.phase]}</strong> of your challenge!</p>

              <div class="success-box">
                <h3>🏆 Phase ${userData.phase} - PASSED!</h3>
                <div style="font-size: 48px; margin: 20px 0;">✅</div>
                <p>You've demonstrated exceptional trading skills and discipline.</p>
              </div>

              <div class="phase-info">
                <h3>Challenge Details</h3>
                <p><strong>Phase Completed:</strong> ${phaseNames[userData.phase]}</p>
                <p><strong>Account Size:</strong> $${userData.accountSize?.toLocaleString() || 'N/A'}</p>
                <p><strong>Completion Date:</strong> ${new Date().toLocaleDateString()}</p>
              </div>

              <p><strong>What's Next?</strong></p>
              <ul>
                <li>${nextSteps[userData.phase]}</li>
                <li>Our team will review your performance</li>
                <li>You'll receive your next phase credentials via email</li>
                ${userData.phase === 2 ? '<li>Start trading with our capital and keep up to 90% of profits!</li>' : ''}
              </ul>

              <div style="text-align: center;">
                <a href="${process.env.FRONTEND_URL || 'https://fund8r.com'}/dashboard" class="button">View Your Progress</a>
              </div>

              <p>Congratulations on this significant milestone! Keep up the excellent work.</p>
              <p>Best regards,<br>The ${process.env.COMPANY_NAME || 'Fund8r'} Team</p>
            </div>
            <div class="footer">
              <p>If you have any questions, please contact our support team.</p>
              <p>&copy; ${new Date().getFullYear()} ${process.env.COMPANY_NAME || 'Fund8r'}</p>
      ...
      console.log(`✅ Phase completion email sent to ${userData.email}`);
    } catch (error) {
  ...
  generateInvoice(filePath, customerName, customerEmail, invoiceNumber, challengeData) {
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(9).fillColor('#999999').text('Questions? Contact urgent.fund8r@gmail.com', 60, doc.page.height - 80, { align: 'center', width: doc.page.width - 120 });
  ...
  // ============================================
  // NEW CANVAS-BASED CERTIFICATE EMAIL METHODS
  // ============================================

  async sendWelcomeWithCertificate(userData) {
    try {
      ...
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Welcome email simulation (SMTP not configured)');
        console.log(`To: ${userData.email}`);
        console.log(`Name: ${userData.name || userData.full_name || 'Trader'}`);
        console.log(`Account ID: ${userData.accountId || userData.friendly_id || 'FX-' + Date.now()}`);
        return;
      }
      ...
      const html = `
        ...
        <body>
          <div class="container">
            ...
            <div class="title">Welcome to Fund8r Forex!</div>
            <div class="content">
              <h2>Hi ${userData.name || userData.full_name || 'Trader'},</h2>
              ...
              <div class="highlight">
                ...
                <p><strong>Account ID:</strong> ${userData.accountId || userData.friendly_id || 'FX-' + Date.now()}</p>
              ...
              <div style="text-align: center;">
                <a href="${process.env.FRONTEND_URL || 'https://fund8r.com'}/dashboard" class="button">Go to Dashboard</a>
              </div>
            </div>
            <div class="footer">
      ...
      await this.transporter.sendMail({
        from: `"${process.env.COMPANY_NAME || 'Fund8r Forex'}" <${process.env.SMTP_USER}>`,
        to: userData.email,
        subject: '🦁 Welcome to Fund8r Forex - Your Trading Journey Begins!',
        html
      });
  ...
  async sendVerificationCode(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Verification email simulation (SMTP not configured)');
      ...
      const html = this.templates.verification ? this.templates.verification({
      ...
      console.log(`✅ Verification code sent to ${userData.email}`);
      return code;
    } catch (error) {
  ...
  async sendSignInCode(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Sign-in code simulation (SMTP not configured)');
        const code = Math.floor(100000 + Math.random() * 900000).toString();
        console.log(`   Code: ${code}`);
        return code;
      }
      ...
      const html = this.templates.signin ? this.templates.signin({
        name: userData.name || 'User',
        code: code,
      ...
      console.log(`✅ Sign-in code sent to ${userData.email}`);
      return code;
    } catch (error) {
  ...
  async sendPassingCertificate(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Passing certificate simulation (SMTP not configured)');
        console.log(`To: ${userData.email}`);
        console.log(`Phase: ${userData.phase || 'Evaluation'}`);
        console.log(`Profit: ${userData.profit || '10%'}`);
        return;
      }
      ...
      const html = `
        <!DOCTYPE html>
        <html>
        <body style="font-family: Arial; background: linear-gradient(135deg, #1E3A8A, #7C3AED); padding: 20px;">
          <div style="max-width: 600px; margin: 0 auto; background: rgba(255,255,255,0.1); border-radius: 15px; padding: 40px; color: white;">
            ...
            <p style="text-align: center; font-size: 18px;">You've passed ${userData.phase || 'the evaluation'}!</p>
            <p>Hi ${userData.name || 'Trader'},</p>
            <p>Outstanding performance! You've successfully completed your challenge.</p>
            <div style="background: rgba(255,215,0,0.1); border: 2px solid #FFD700; border-radius: 10px; padding: 20px; margin: 20px 0; text-align: center;">
              <div style="font-size: 24px; color: #FFD700; font-weight: bold;">${userData.profit || '10%'}</div>
              <div style="font-size: 14px;">Profit Achieved</div>
            </div>
      ...
      console.log(`✅ Passing certificate sent to ${userData.email}`);
    } catch (error) {
  ...
  async sendPayoutNotification(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Payout notification simulation (SMTP not configured)');
        console.log(`To: ${userData.email}`);
        console.log(`Amount: $${userData.amount || '1,000.00'}`);
        return;
      }
      ...
      const html = `
        ...
        <body style="font-family: Arial; background: linear-gradient(135deg, #1E3A8A, #7C3AED); padding: 20px;">
          <div style="max-width: 600px; margin: 0 auto; background: rgba(255,255,255,0.1); border-radius: 15px; padding: 40px; color: white;">
            <div style="text-align: center; font-size: 60px; margin-bottom: 20px;">🦁</div>
            <h1 style="color: #FFD700; text-align: center;">Payout Approved!</h1>
            <p>Congratulations ${userData.name || 'Trader'}!</p>
            <p>Your payout has been processed successfully.</p>
            <div style="background: linear-gradient(135deg, rgba(255,215,0,0.2), rgba(255,215,0,0.1)); border: 3px solid #FFD700; border-radius: 15px; padding: 30px; margin: 30px 0; text-align: center;">
              <div style="font-size: 48px; color: #FFD700; font-weight: bold;">$${userData.amount || '1,000.00'}</div>
              <div style="background: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; font-family: monospace; margin: 20px 0;">
                Transaction ID: ${userData.transactionId || 'PAY-' + Date.now()}
              </div>
              <p>Expected arrival: ${userData.arrivalTime || '1-3 business days'}</p>
            </div>
            <p style="text-align: center;">Your payout has been processed and will arrive soon.</p>
          </div>
        </body>
        </html>
      `;
      ...
      console.log(`✅ Payout notification sent to ${userData.email}`);
    } catch (error) {
  ...
  async sendSupportReply(data) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 EMAIL SIMULATION - Support Reply Email');
        console.log(`To: ${data.email}`);
        console.log(`Subject: Re: ${data.subject}`);
      ...
      const html = `
        ...
        <head>
          <style>
            ...
            .content { padding: 30px; }
            .message-box { background: #f8f9fa; border-left: 4px solid #7C3AED; padding: 20px; margin: 20px 0; border-radius: 5px; }
            .footer { background: #f8f9fa; padding: 20px; text-align: center; color: #666; font-size: 12px; }
            .btn { display: inline-block; padding: 12px 30px; background: linear-gradient(135deg, #3B82F6, #7C3AED); color: white; text-decoration: none; border-radius: 5px; margin: 20px 0; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>You can view and reply to this ticket in your dashboard:</p>
              <a href="https://fund8r.com/dashboard" class="btn">View Ticket</a>
              <p>If you have any questions, feel free to reply to this ticket.</p>
              <p>Best regards,<br>Fund8r Support Team</p>
            </div>
            <div class="footer">
              <p>This is an automated message from Fund8r Support System</p>
      ...
      await this.transporter.sendMail({
        from: `"Fund8r Support" <${process.env.SMTP_USER}>`,
        to: data.email,
        subject: `Re: ${data.subject}`,
        html
      });

      console.log(`✅ Support reply sent to ${data.email}`);
    } catch (error) {
  ...
  async sendChallengeStartedEmail(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Challenge Started Email Simulation (SMTP not configured)');
        console.log(`To: ${userData.email}`);
        console.log(`Challenge: ${userData.challenge_type} - $${userData.account_size}`);
        return;
      }
      ...
      const html = `
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1>🚀 Your Challenge Has Started!</h1>
              <p>Welcome to the trading battlefield</p>
            </div>
            <div class="content">
              <h2>Hi ${userData.name || 'Trader'},</h2>
              <p>Your <strong>${userData.challenge_type}</strong> challenge with a <strong>$${userData.account_size.toLocaleString()}</strong> account is now active!</p>

              <div class="credentials-box">
                <h3>📋 Your MT5 Credentials</h3>
                <div class="credential-item"><strong>Login:</strong> <span>${userData.mt5_login || 'TBD'}</span></div>
                <div class="credential-item"><strong>Password:</strong> <span>${userData.mt5_password || 'TBD'}</span></div>
                <div class="credential-item"><strong>Server:</strong> <span>${userData.mt5_server || 'MetaQuotes-Demo'}</span></div>
                <div class="credential-item"><strong>Account Size:</strong> <span>$${userData.account_size.toLocaleString()}</span></div>
              </div>

              <p><strong>📈 Challenge Rules:</strong></p>
              <ul>
                <li>Profit Target: 8% for Phase 1, 5% for Phase 2</li>
                <li>Maximum Daily Loss: 5%</li>
                <li>Maximum Total Loss: 10%</li>
              ...
              <p>Good luck with your challenge! Remember, consistency is key to success.</p>
              <p>Best regards,<br>The ${process.env.COMPANY_NAME || 'Fund8r'} Team</p>
            </div>
            <div class="footer">
              <p>If you have any questions, please contact our support team.</p>
      ...
      await this.transporter.sendMail({
        from: `"${process.env.COMPANY_NAME || 'Fund8r'}" <${process.env.SMTP_USER}>`,
        to: userData.email,
        subject: `🚀 Your ${userData.challenge_type} Challenge Has Started!`,
        html
      });

      console.log(`✅ Challenge started email sent to ${userData.email}`);
    } catch (error) {
  ...
  async sendChallengePassedEmail(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Challenge Passed Email Simulation (SMTP not configured)');
        console.log(`To: ${userData.email}`);
        console.log(`Challenge: ${userData.challenge_type} - $${userData.account_size}`);
        return;
      }
      ...
      const html = `
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1>🎉 Congratulations!</h1>
              <p>You've passed your challenge!</p>
            </div>
            <div class="content">
              <h2>Hi ${userData.name || 'Trader'},</h2>
              <p>Fantastic news! You've successfully passed your <strong>${userData.challenge_type}</strong> challenge with a <strong>$${userData.account_size.toLocaleString()}</strong> account!</p>

              <div class="success-box">
                <h3>🏆 Challenge Completed!</h3>
                <div class="amount">PASSED</div>
                <p>Your trading skills have earned you the right to trade with our capital.</p>
              </div>

              <p><strong>What's Next?</strong></p>
              <ul>
                <li>Our team will review your account within 24-48 hours</li>
                <li>You'll receive your funded account credentials</li>
                <li>Start trading with up to 90% profit split!</li>
              </ul>

              <div style="text-align: center;">
                <a href="${process.env.FRONTEND_URL || 'https://fund8r.com'}/dashboard" class="button">View Your Certificate</a>
              </div>

              <p>Congratulations again on this incredible achievement! You're now part of the elite group of funded traders.</p>
              <p>Best regards,<br>The ${process.env.COMPANY_NAME || 'Fund8r'} Team</p>
            </div>
            <div class="footer">
              <p>If you have any questions, please contact our support team.</p>
      ...
      await this.transporter.sendMail({
        from: `"${process.env.COMPANY_NAME || 'Fund8r'}" <${process.env.SMTP_USER}>`,
        to: userData.email,
        subject: `🎉 Congratulations! You Passed Your ${userData.challenge_type} Challenge!`,
        html
      });

      console.log(`✅ Challenge passed email sent to ${userData.email}`);
    } catch (error) {
  ...
  async sendBreachNotificationEmail(userData) {
    try {
      if (!this.isConfigured || !this.transporter) {
        console.log('📧 Breach Notification Email Simulation (SMTP not configured)');
        console.log(`To: ${userData.email}`);
        console.log(`Challenge: ${userData.challenge_type} - $${userData.account_size}`);
        return;
      }
      ...
      const html = `
        ...
        <head>
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; background-color: #f4f7f6; }
            .container { max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
            .header { background: linear-gradient(135deg, #dc3545 0%, #c9302c 100%); color: white; padding: 40px; text-align: center; }
            .content { padding: 30px; }
            .breach-box { background: #f2dede; border: 1px solid #ebccd1; border-left: 5px solid #a94442; padding: 20px; margin: 20px 0; border-radius: 5px; color: #a94442; }
            .button { display: inline-block; padding: 15px 30px; background: #667eea; color: white; text-decoration: none; border-radius: 50px; margin-top: 20px; font-weight: bold; }
            .footer { background: #e9ecef; padding: 20px; text-align: center; font-size: 12px; color: #6c757d; }
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1>⚠️ Account Breach Notification</h1>
              <p>Important account information</p>
            </div>
            <div class="content">
              <h2>Hi ${userData.name || 'Trader'},</h2>
              <p>We are writing to inform you that your <strong>${userData.challenge_type}</strong> challenge with a <strong>$${userData.account_size.toLocaleString()}</strong> account has been breached.</p>

              <div class="breach-box">
                <h3>💀 Account Status: BREACHED</h3>
                <p>This means the account can no longer be traded and the challenge has ended.</p>
                <p><strong>Reason:</strong> The account exceeded the maximum allowed drawdown limits.</p>
              </div>

              <p><strong>What happens now?</strong></p>
              <ul>
                <li>The account has been suspended</li>
                <li>You can view the breach details in your dashboard</li>
                <li>You can purchase a new challenge anytime</li>
                <li>All your trading data is preserved for review</li>
              </ul>

              <div style="text-align: center;">
                <a href="${process.env.FRONTEND_URL || 'https://fund8r.com'}/dashboard" class="button">View Dashboard</a>
              </div>

              <p>Don't be discouraged! Many successful traders faced breaches before achieving funded status. Consider our educational resources to improve your strategy.</p>
              <p>Best regards,<br>The ${process.env.COMPANY_NAME || 'Fund8r'} Team</p>
            </div>
            <div class="footer">
              <p>If you believe this is an error, please contact our support team immediately.</p>
      ...
      await this.transporter.sendMail({
        from: `"${process.env.COMPANY_NAME || 'Fund8r'}" <${process.env.SMTP_USER}>`,
        to: userData.email,
        subject: `⚠️ Account Breach Notification - ${userData.challenge_type}`,
        html
      });

      console.log(`✅ Breach notification email sent to ${userData.email}`);
    } catch (error) {
