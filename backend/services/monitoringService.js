...
class MonitoringService {
  ...
  async checkRuleViolations(account, metrics, deals) {
    ...
    if (account.challenge_type === 'MINI_FREE') {
      // Mini Challenge Rules
      if (metrics.daily_drawdown > 3) {
        violations.push({ rule: 'daily_drawdown', value: metrics.daily_drawdown, limit: 3, severity: 'critical' });
      }
      if (metrics.max_drawdown > 5) {
        violations.push({ rule: 'max_drawdown', value: metrics.max_drawdown, limit: 5, severity: 'critical' });
      }
      // Cannot risk more than 0.5% per trade
      const riskPerTradeViolation = this.checkRiskPerTrade(deals, account.balance);
      if (riskPerTradeViolation) {
        violations.push(riskPerTradeViolation);
      }
      // Cannot move the stop loss
      const moveStopLossViolation = this.checkMoveStopLoss(deals);
      if (moveStopLossViolation) {
        violations.push(moveStopLossViolation);
      }
      // Cannot make more than 2% a day
      const maxDailyGainViolation = this.checkMaxDailyGain(deals, account.balance);
      if (maxDailyGainViolation) {
        violations.push(maxDailyGainViolation);
      }
    } else {
    ...
    if (challenge && challenge.min_trading_days && metrics.trading_days < challenge.min_trading_days) {
      violations.push({
    ...
    if (challenge && challenge.consistency_threshold && metrics.consistency_score < challenge.consistency_threshold) {
      violations.push({
        rule: 'consistency',
  ...
  checkRiskPerTrade(deals, balance) {
    for (const deal of deals) {
      if (deal.type === 'buy' || deal.type === 'sell') {
        const risk = (deal.price - deal.stopLoss) * deal.volume;
        const riskPercentage = (risk / balance) * 100;
  ...
  checkMoveStopLoss(deals) {
    // This is a complex rule to implement and would require historical data
    // For now, we will return null
    return null;
  }

  checkMaxDailyGain(deals, balance) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    const todayProfit = todayDeals.reduce((sum, deal) => sum + deal.profit, 0);
    const dailyGainPercentage = (todayProfit / balance) * 100;

    if (dailyGainPercentage > 2) {
      return {
        rule: 'max_daily_gain',
        value: dailyGainPercentage,
        limit: 2,
        severity: 'critical'
      };
    }
